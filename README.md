...
| **Backend** | Flask 3.1 · SQLAlchemy · MySQL 8.0 / TiDB · gunicorn |
| **Maps & Geo** | Leaflet 1.9 · Stadia Maps (Stamen Watercolor) · Google Places · OSM Nominatim · GSI |
| **Infra** | Google Cloud Run · Docker · Cloud Storage · Secret Manager · Google OAuth 2.0 · GitHub Actions |
...
- **Existence check for restaurants & hotels**: with `GOOGLE_MAPS_API_KEY` set, candidate names are verified against Google Places at generation time and LLM-invented names are dropped from the candidate pool (the filter backs off if too few verified candidates remain).
...
| `GOOGLE_MAPS_API_KEY` | | Optional Google Places-powered geocoding boost: restaurant/hotel names missing from OSM get pins on the map. Unset = free stack (Nominatim + GSI) only |
