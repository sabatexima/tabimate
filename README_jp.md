...
START
  ...
  → balancer                   全体の見直し
        ...
        └─ fix_* → 該当ノードへ戻る   （上限: MAX_BALANCER_RETRIES = 5）
             1回目は選び直し（安い）／同じ指摘が続いたら候補集めまで戻る（高い）
```
...
- **差し戻しの段階** — 1回目は同じ候補プールから選び直します（安い）。同じ指摘が2回続いたら、プールの中に正解が無いということなので、候補集めまで戻って顔ぶれを入れ替えます（検索とLLMを使うので高い）。候補エージェントには審査の指摘と前回弾かれた顔ぶれを渡します。渡さないと temperature=0 なので同じ候補が返ってくるだけです。
...
```bash
pytest tests/ -k "not smoke"    # 183件のオフラインテスト（APIキーもDBも不要）
...
| `test_regression.py`（54） | 一度戻ってきたことのある不具合。テンプレートの `url_for` が実在の endpoint を指すこと、プランカードに空行が無いこと、公開リンクの旅で写真が枠に入ること、「運転しない」が保存と修正の往復で消えないことなど |
