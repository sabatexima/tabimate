...
- **スポットの地図表示**：保存プランに水彩スタイルの地図（Leaflet + Stadia Maps / Stamen Watercolor タイル）を表示。観光は番号付きの緑のしずく型ピン（マスコット由来の四つ葉アクセント付き）でルート順に結び、グルメ（オレンジ）・宿（青）は色分けピン＋凡例で表示する。各ピンのポップアップから Google マップの経路ナビへ飛べる。緯度経度は**地図を初めて開いたとき**にジオコーディングしてキャッシュ（`geo_done`）。目的地周辺を優先（`viewbox`）し、`名前 → 「名前, 目的地」 → 末尾総称トリム` の順でフォールバックしてヒット率を上げる。共有プランの地図や写真の足あとマップも同じ部品を再利用する。
...
- **旅行日の天気・カレンダー書き出し**：保存（と共有）プランに旅行日の天気予報ストリップを表示し、`.ics` カレンダーファイル（Google カレンダー等に取込）として書き出せる。
