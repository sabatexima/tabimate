...
- **スポットの地図表示**：保存プランに水彩スタイルの地図（Leaflet + Stadia Maps / Stamen Watercolor タイル）を表示。観光は番号付きの緑のしずく型ピン（マスコット由来の四つ葉アクセント付き）でルート順に結び、グルメ（オレンジ）・宿（青）は色分けピン＋凡例で表示する。各ピンのポップアップから Google マップの経路ナビへ飛べる。緯度経度は**地図を初めて開いたとき**にジオコーディングしてキャッシュ（`geo_done`）。精度向上のため、目的地周辺を優先（`viewbox`）した上で、表記の正規化（NFKC）→ 括弧注釈・末尾総称を外した再検索 → 「名前, 目的地」での再検索を行い、候補は複数取得して**目的地に最も近いものを採用**（遠すぎる同名別地は棄却）。Nominatim で当たらない地名は**国土地理院の住所検索APIにフォールバック**する（どちらも無料・キー不要）。共有プランの地図や写真の足あとマップも同じ部品を再利用する。
